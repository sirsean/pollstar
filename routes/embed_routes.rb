get '/poll/:poll_id/embed.js' do |poll_id|
    @logger.debug "Embedding poll (javascript): #{poll_id}"
...
get '/poll/:poll_id/embed/' do |poll_id|
    @logger.debug "Embedding poll: #{poll_id}"
    if @env["rack.request.query_hash"]["width"]
        @width = @env["rack.request.query_hash"]["width"].to_i
    else
        @width = 570
    ...
    if @current_user
        ...
        @logger.debug "logged in user voted: #{@voted}"
    else
        @voted = Vote.has_ip_voted_on_poll?(@ip, @poll.id)
        @logger.debug "anonymous user voted: #{@voted}"
    end
